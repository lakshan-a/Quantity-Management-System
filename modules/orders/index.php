...
<script src="../../assets/js/orders/translations.js"></script>
    ...
    if (filtered.length) {
      mobileContainer.innerHTML = filtered.map(order => `
        <div class="p-4 space-y-3">
          ...
          <div class="grid grid-cols-2 gap-2 text-sm"><div><span class="text-slate-500 text-xs block">Customer</span><span>${escapeHtml(order.customer_name)}</span></div><div><span class="text-slate-500 text-xs block">Total</span><span>$${order.total_amount.toFixed(2)}</span></div><div><span class="text-slate-500 text-xs block">Payment</span><span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${paymentStatusColors[order.payment_status]}">${order.payment_status}</span></div><div><span class="text-slate-500 text-xs block">Date</span><span>${formatDate(order.createdAt)}</span></div></div>
