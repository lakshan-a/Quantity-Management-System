...
<style>
    ...
    .modal-content-enter-active { transition: transform 0.2s ease, opacity 0.2s ease; }
    ...
    <div id="app" class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="relative w-full sm:w-80">
                <i class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 fas fa-search text-sm"></i>
                <input type="text" id="searchInput" placeholder="Search items by name or code..." class="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div class="flex gap-3">
                ...
                <div class="relative" id="filterDropdown">
                    <button id="filterButton" class="inline-flex items-center px-4 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                        <i class="fas fa-sliders-h mr-2 text-slate-500"></i> Filter
                    ...
                    <div id="filterMenu" class="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 z-20 hidden">
                        <div class="p-4 space-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Stock Level</label>
                            ...
                            <button id="resetFiltersBtn" class="w-full px-3 py-2 text-sm text-blue-600 dark:text-blue-400 border border-blue-300 dark:border-blue-600 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors">Reset Filters</button>
                        </div>
                    </div>
                </div>
                <button id="openAddItemBtn" class="inline-flex items-center justify-center px-4 py-2 bg-blue-500 text-white font-medium rounded-lg hover:bg-blue-600 transition-colors shadow-sm">
                    <i class="fas fa-plus mr-2 w-4 h-4"></i> Add Item
                </button>
            </div>
        </div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Stock</th>
    ...
    <div class="fixed inset-0 flex flex-col sm:items-center sm:justify-center sm:p-4">
        ...
        <div class="relative w-full bg-white dark:bg-slate-800 shadow-xl rounded-t-2xl sm:rounded-xl sm:max-w-2xl max-h-[90vh] overflow-hidden transform transition-all">
            <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700">
                ...
                <h2 id="modalTitle" class="text-lg font-semibold text-slate-900 dark:text-white pt-2 sm:pt-0">Add Item</h2>
            ...
            <div class="p-4 overflow-y-auto max-h-[calc(90vh-140px)]">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Item Code</label>
                        ...
                        <p id="codeHelper" class="text-xs text-slate-500 mt-1">Auto-generated</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Item Name *</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Wholesale</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Size</label>
                        <input type="text" id="size" placeholder="e.g., M, L, XL, One Size" class="w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Colors</label>
                        <input type="text" id="colors" placeholder="Red, Blue, Green" class="w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Cost Price</label>
                        <div class="relative">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Selling Price</label>
                        <div class="relative">
                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500">$</span>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Stock Quantity</label>
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Item Image</label>
            ...
            <div class="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2 sm:gap-3 p-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                <button id="cancelModalBtn" class="px-4 py-2 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 font-medium rounded-lg hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors">Cancel</button>
                <button id="saveItemBtn" class="px-4 py-2 bg-blue-500 text-white font-medium rounded-lg hover:bg-blue-600 transition-colors shadow-sm">Save Item</button>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="relative w-full bg-white dark:bg-slate-800 shadow-xl rounded-t-2xl sm:rounded-xl sm:max-w-lg max-h-[90vh] overflow-hidden">
            <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700">
                <h2 class="text-lg font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                    <i class="fas fa-box text-blue-500"></i> Item Details
                </h2>
    ...
    <div class="fixed inset-0 flex flex-col sm:items-center sm:justify-center sm:p-4">
        ...
        <div class="relative w-full bg-white dark:bg-slate-800 shadow-xl rounded-t-2xl sm:rounded-xl sm:max-w-md overflow-hidden transform transition-all">
            <div class="p-6">
                ...
                <h3 class="text-lg font-semibold text-center text-slate-900 dark:text-white mb-2">Delete Item</h3>
                <p class="text-center text-slate-500 dark:text-slate-400 mb-6">Are you sure you want to delete this item? This action cannot be undone.</p>
                <div class="flex gap-3">
            ...
            <div class="p-4 space-y-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center overflow-hidden flex-shrink-0">
                        ${item.item_image ? `<img src="${item.item_image}" class="w-full h-full object-cover">` : `<i class="fas fa-box text-slate-400 text-xl"></i>`}
                    </div>
                ...
                <div class="flex gap-2 pt-2 border-t border-slate-100 dark:border-slate-700 mt-2">
                    <button data-id="${item.item_id}" data-action="view" class="action-btn p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors"><i class="fas fa-eye text-slate-600 dark:text-slate-400"></i></button>
                    <button data-id="${item.item_id}" data-action="edit" class="action-btn p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors"><i class="fas fa-edit text-slate-600 dark:text-slate-400"></i></button>
                    <button data-id="${item.item_id}" data-action="delete" class="action-btn p-2 hover:bg-red-100 dark:hover:bg-red-900/30 rounded-lg transition-colors"><i class="fas fa-trash-alt text-red-500"></i></button>
                </div>
            </div>
        `).join('');
            ...
            <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                <td class="px-4 py-3">
                    <div class="w-10 h-10 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center overflow-hidden">
                        ${item.item_image ? `<img src="${item.item_image}" class="w-full h-full object-cover">` : `<i class="fas fa-box text-slate-400 text-lg"></i>`}
                    </div>
                </td>
                ...
                <td class="px-4 py-3">
                    <div class="flex gap-2">
                        <button data-id="${item.item_id}" data-action="view" class="action-btn p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors" title="View Details">
                            <i class="fas fa-eye text-slate-600 dark:text-slate-400"></i>
                        </button>
                        <button data-id="${item.item_id}" data-action="edit" class="action-btn p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors" title="Edit Item">
                            <i class="fas fa-edit text-slate-600 dark:text-slate-400"></i>
                        </button>
    ...
    function showViewModal(item) {
            ...
            <div class="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center ${m.type === 'in' ? 'bg-emerald-100 dark:bg-emerald-900/30' : 'bg-red-100 dark:bg-red-900/30'}">
                        <i class="fas fa-arrow-down ${m.type === 'in' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'} transform ${m.type === 'out' ? 'rotate-180' : ''}"></i>
                    </div>
                    <div>
        ...
        const detailsHtml = `
            <div class="w-full h-48 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center overflow-hidden">
                ${item.item_image ? `<img src="${item.item_image}" class="w-full h-full object-cover">` : `<i class="fas fa-box text-6xl text-slate-400"></i>`}
            </div>
