...
<div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6" id="damagesApp">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div class="relative w-full sm:w-72">
        ...
        <input type="text" id="searchInput" placeholder="Search by item or reason..." class="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500" data-i18n-placeholder="search_damages">
      </div>
      <button id="openModalBtn" class="inline-flex items-center justify-center px-4 py-2 bg-blue-500 text-white font-medium rounded-lg hover:bg-blue-600 transition-colors">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-2"><path d="M12 5v14M5 12h14"/></svg>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
      <!-- mobile cards (hidden md:block) but we show/hide via responsive -->
      ...
      <div id="emptyMessageContainer" class="p-8 text-center text-slate-500 hidden" data-i18n="no_damages_found">No damaged records found.</div>
    </div>
  </div>

  <!-- MODAL STRUCTURE for Add/Edit (hidden by default) -->
    ...
    <div class="fixed inset-0 flex flex-col sm:items-center sm:justify-center sm:p-4">
      ...
      <div class="relative w-full bg-white dark:bg-slate-800 shadow-xl rounded-t-2xl sm:rounded-xl sm:max-w-md max-h-[90vh] overflow-hidden">
        <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700">
          ...
          <h2 id="modalTitle" class="text-lg font-semibold text-slate-900 dark:text-white pt-2 sm:pt-0" data-i18n="report_damage">Report Damage</h2>
        ...
        <div class="p-4 space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5" data-i18n="item_required">Item *</label>
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5" data-i18n="quantity_required">Quantity *</label>
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5" data-i18n="reason_required">Reason *</label>
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5" data-i18n="reported_by_required">Reported by *</label>
        ...
        <div class="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2 sm:gap-3 p-4 border-t border-slate-200 dark:border-slate-700">
          ...
          <button id="modalSaveBtn" class="px-4 py-2 bg-blue-500 text-white font-medium rounded-lg hover:bg-blue-600 transition-colors" data-i18n="report_damage">Report Damage</button>
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="fixed inset-0 flex flex-col sm:items-center sm:justify-center sm:p-4">
        ...
        <div class="relative w-full bg-white dark:bg-slate-800 shadow-xl rounded-t-2xl sm:rounded-xl sm:max-w-md overflow-hidden">
            <div class="p-6">
                ...
                <h3 class="text-lg font-semibold text-center text-slate-900 dark:text-white mb-2" data-i18n="delete_damage_record">Delete Damage Record</h3>
                <p class="text-center text-slate-500 dark:text-slate-400 mb-6"><span data-i18n="delete_confirmation_message">Are you sure you want to delete the damage record for</span> <span id="deleteItemName" class="font-medium text-slate-700 dark:text-slate-300"></span>? <span data-i18n="delete_confirmation_warning">This action cannot be undone.</span></p>
  ...
  <script>
    ...
    // Helper: filter damages based on search
    function getFilteredDamages() {
      if (!searchQuery.trim()) return [...damages];
    ...
    function renderUI() {
      ...
      if (filtered.length === 0) {
        ...
        emptyRow.innerHTML = `<td colspan="7" class="px-4 py-8 text-center text-slate-500">No damaged records found.</td>`;
        tableBody.appendChild(emptyRow);
        mobileContainer.innerHTML = `<div class="p-8 text-center text-slate-500">No damaged records found.</div>`;
      ...
      filtered.forEach(d => {
        ...
        row.innerHTML = `
          ...
          <td class="px-4 py-3 text-sm text-slate-600 dark:text-slate-300 max-w-xs truncate">${escapeHtml(d.reason)}</td>
          ...
          <td class="px-4 py-3 text-right">
            <div class="flex items-center justify-end gap-2">
              ...
              <button class="view-btn p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors" data-id="${d.damage_id}" title="View">
              ...
              <button class="edit-btn p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors" data-id="${d.damage_id}" title="Edit">
              ...
              <button class="delete-btn p-2 hover:bg-red-100 dark:hover:bg-red-900/30 rounded-lg transition-colors" data-id="${d.damage_id}" data-name="${escapeHtml(d.item_name)}" title="Delete">
      ...
      filtered.forEach(d => {
        ...
        card.innerHTML = `
          ...
          <div class="grid grid-cols-2 gap-2 text-sm">
            <div><span class="text-slate-500 text-xs block">Quantity</span><span class="font-medium text-red-600 dark:text-red-400">${d.quantity}</span></div>
            <div><span class="text-slate-500 text-xs block">Item Code</span><span class="text-slate-700 dark:text-slate-300 font-mono text-xs">${escapeHtml(getItemCode(d.item_id))}</span></div>
            <div><span class="text-slate-500 text-xs block">Reported date</span><span class="text-slate-700 dark:text-slate-300">${formatDate(d.createdAt)}</span></div>
            <div class="col-span-2"><span class="text-slate-500 text-xs block">Reason</span><span class="text-slate-700 dark:text-slate-300">${escapeHtml(d.reason)}</span></div>
            <div class="col-span-2"><span class="text-slate-500 text-xs block">Reported by</span><span class="text-slate-700 dark:text-slate-300">${escapeHtml(d.reported_by)}</span></div>
          </div>
        `;
    ...
    function editDamage(id) {
      ...
      document.getElementById('modalTitle').innerText = 'Edit Damage Report';
      document.getElementById('modalSaveBtn').innerHTML = 'Update Damage';
    ...
    function resetModalForm() {
      ...
      document.getElementById('modalSaveBtn').innerHTML = 'Report Damage';
