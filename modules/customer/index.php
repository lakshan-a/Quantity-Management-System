    ...
    mobileContainer.innerHTML = filtered.length ? filtered.map(c => `
       <div class="p-4 space-y-3">
                ...
                <div class="grid grid-cols-2 gap-2 text-sm">
                    <div><span class="text-slate-500 text-xs block">${t('table_header_email')}</span><span class="text-slate-700 dark:text-slate-300">${escapeHtml(c.email)}</span></div>
                    <div><span class="text-slate-500 text-xs block">${t('table_header_phone')}</span><span class="text-slate-700 dark:text-slate-300">${escapeHtml(c.phone)}</span></div>
                    <div><span class="text-slate-500 text-xs block">${t('table_header_city')}</span><span class="text-slate-700 dark:text-slate-300">${escapeHtml(c.city || '—')}</span></div>
                </div>
                <div class="flex items-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-700">
                    <button onclick="viewCustomer('${c.customer_id}')" class="p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg" title="${t('view')}">
                    ...
                    <button onclick="openEditCustomer('${c.customer_id}')" class="p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg" title="${t('edit')}">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                    </button>
                    <button onclick="deleteCustomerPrompt('${c.customer_id}')" class="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg" title="${t('delete')}">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                    </button>
                </div>
            </div>
        `).join('') : `<div class="p-8 text-center text-slate-500">${t('no_customers_found')}</div>`;
  }
